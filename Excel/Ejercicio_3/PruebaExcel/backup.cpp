...
void PruebaExcel::Cmd_Msexcel(Win::Event& e)
{
	...
	Win::FileDlg dlg;
	dlg.Clear();
	dlg.SetFilter(L"Excel files (*.xlsm)\0*.xlsm\0\0", 0, L"xlsm");
	...
	try
	{
		...
		//Cambia a otra hoja de trabajo
		Aplicacion.ExecuteExcel4Macro(L"MESN"); //Usando Macro
		Hoja = Aplicacion.ActiveSheet;

		//Pone un rango
		Rango = Hoja.get_Range(L"A4");
		Rango.Select();
